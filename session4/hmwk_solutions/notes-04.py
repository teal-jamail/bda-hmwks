...
# ── Space: O(n) - all image file handles exist at once

# ── QUIZ NOTES ──────────────────────────────────────────
# ── 'join()': blocks main process and waits for child to finish
#    makes sure main process doesnt move on b4 child is done

# ── 'mp.Process': create seperate OS process
#    an independent py interpreter w/ own memory, OS managed
